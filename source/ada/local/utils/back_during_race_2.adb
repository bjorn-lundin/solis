...
procedure Back_During_Race_2 is
  ...
  begin
    if Bet_List.Length = 0 then -- ie no previous bet in this race
      ...
      if Bra(3).Selectionid > Integer_4(0) and then  -- sanity
        Bra(3).Backprice    >= Fixed_Type(1.0) and then  -- sanity
       ...
       -- Bra(3).Layprice     <= Fixed_Type(Five * Bra(3).Backprice) and then -- not too big difference allowed
        Bra(3).Backprice    >= Bra(1).Backprice + Fixed_Type(Delta_Price) and then
