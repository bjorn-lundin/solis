...
procedure Price_Graph_During_Race_Year_Month is
...
--    Debug ("Get_Runners - Place market is found");
  ...
  ----------------------------------------------
  function Zero_Fill( S: String) return String is
  begin
    case S'Length is
      when 1      => return "0" & S;
      when 2      => return S;
      when others => return "wtf-" & S & "!!!-";
    end case;
  end Zero_Fill;
 ...
 --   Debug ("Print_To_File - start " & Placement'Img);
    ...
    for R of Rl loop
      ...
      if Phl.Length > 200 then
        for Ph of Phl loop
           Filename.Set("/home/bnl/bnlbot/botstart/bot-1-0/script/plot/price_plots_month/dats/" &
                       Trim(M.Startts.Year'Img) & "_" &
                       Zero_Fill(Trim(M.Startts.Month'Img)) & "_" &
                       F8_Image(Ph.Backprice) & "_" &
                       M.Marketid & "_" & Trim(R.Selectionid'Img) & "_" & Placement'Img & ".dat");
           exit; -- only to get first odds
        end loop;
...
end Price_Graph_During_Race_Year_Month;
