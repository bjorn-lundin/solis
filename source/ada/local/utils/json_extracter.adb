         ...
         for I in 1 .. Length (Time_Series_Array) loop
            for k in 1 .. 12 loop
               ...
               if Data_Item.Has_Field ("time") then
                  ...
                  begin
                     SMHI_Data.time_stop := Calendar2.To_Time_Type (tm(1..19) & ".000");
                     Log ("time " & Tm);
                  end;
               end if;
               ...
               if Data_Item.Has_Field ("intervalParametersStartTime") then
                  ...
                  begin
                     SMHI_Data.time_start := Calendar2.To_Time_Type (ipst(1..19) & ".000");
                    -- Log ("k " & k'img);
                     SMHI_Data.time_start := SMHI_Data.time_start - (0, 0, Minute_Type((k-1)*5), 0, 0); -- every 5 minutes
                     Log ("intervalParametersStartTime " & ipst);
                     Map_To_Epoch (SMHI_Data.time_start, Epoch);
                     SMHI_Data.epoch := Integer_4(Epoch);
                     SMHI_Data.time_start := Calendar2.To_Time_Type (ipst(1..19) & ".000"); --reset
                  end;
               end if;
         ...
         for k in 1 .. 4 loop  -- each 15 min divided into 4 parts of 5 min
            ...
            if data_item.Has_Field ("time_start") then
               ...
               begin
                  ...
                  Log ("time_start i" & I'img & " k" & k'img & " " & ts);
                  Price_Data.Time_Start := Price_Data.Time_Start - (0, 0, Minute_Type((k-1)*15), 0, 0); -- every 15 minutes
                  Map_To_Epoch (Price_Data.Time_Start, Epoch);
                  Price_Data.epoch := Integer_4(Epoch);
                  Price_Data.Time_Start := Calendar2.To_Time_Type (ts(1..19) & ".000"); --reset
               end;
            end if;
